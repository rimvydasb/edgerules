...
const runLoanDecisionExample = () => {
    ...
    console.log('Loan decision WASM example completed.\n');
};

const runInvocationExample = () => {
    console.log('Running invocation example (defaults and linking)...');
    wasm.create_decision_service(INVOCATION_MODEL);

    const response = wasm.execute_decision_service('buildResponse', {score: 705});
    expect(response.summary.approved === true, 'Invocation should call evaluateEligibility');
    expect(response.score === 705, 'buildResponse should echo the request score');
    ...
    console.log('Stored invocation via set_invocation:', invocationEcho);
    expect(
        invocationEcho['@method'] === 'evaluateEligibility',
        'set_invocation should return the stored invocation snippet'
    );
    ...
    try {
        ...
        linkErrored = true;
        console.log('Expected linking error from set_invocation:', err.message);
    }
    expect(linkErrored, 'Linking errors should bubble when invocation cannot be linked');
};

const runNestedFunctionInsertionExample = () => {
    console.log('Running nested function insertion example...');
    ...
    console.log('Nested function decision:', decision);
    expect(decision.isEligible === true, 'Outer function should still evaluate eligibility');
    expect(
        decision.scholarship.result === 1000,
        'Invocation should reuse inner function and compute scholarship'
    );
};
